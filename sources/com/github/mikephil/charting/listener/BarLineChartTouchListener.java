...
public class BarLineChartTouchListener extends ChartTouchListener<BarLineChartBase<? extends BarLineScatterCandleBubbleData<? extends IBarLineScatterCandleBubbleDataSet<? extends Entry>>>> {
            ...
            switch (event.getAction() & 255) {
                ...
                case 1:
                    ...
                    if ((Math.abs(velocityX) > ((float) Utils.getMinimumFlingVelocity()) || Math.abs(velocityY) > ((float) Utils.getMinimumFlingVelocity())) && this.mTouchMode == 1 && ((BarLineChartBase) this.mChart).isDragDecelerationEnabled()) {
                        ...
                        this.mDecelerationCurrentPoint.f72x = event.getX();
                        this.mDecelerationCurrentPoint.f73y = event.getY();
                        this.mDecelerationVelocity.f72x = velocityX;
                        this.mDecelerationVelocity.f73y = velocityY;
                ...
                case 2:
                    if (this.mTouchMode != 1) {
                        if (this.mTouchMode != 2 && this.mTouchMode != 3 && this.mTouchMode != 4) {
                            if (this.mTouchMode == 0 && Math.abs(distance(event.getX(), this.mTouchStartPoint.f72x, event.getY(), this.mTouchStartPoint.f73y)) > this.mDragTriggerDist) {
    ...
    private void saveTouchStart(MotionEvent event) {
        ...
        this.mTouchStartPoint.f72x = event.getX();
        this.mTouchStartPoint.f73y = event.getY();
    ...
    private void performDrag(MotionEvent event) {
        ...
        if (!inverted()) {
            dX = event.getX() - this.mTouchStartPoint.f72x;
            dY = event.getY() - this.mTouchStartPoint.f73y;
    ...
    private void performZoom(MotionEvent event) {
        ...
        if (event.getPointerCount() >= 2) {
            ...
            if (totalDist > this.mMinScalePointerDistance) {
                MPPointF t = getTrans(this.mTouchPointCenter.f72x, this.mTouchPointCenter.f73y);
                ...
                if (this.mTouchMode == 4) {
                    ...
                    if (canZoomMoreY2 || canZoomMoreX2) {
                        this.mMatrix.set(this.mSavedMatrix);
                        this.mMatrix.postScale(scaleX, scaleY, t.f72x, t.f73y);
                        if (l != null) {
                    ...
                    if (canZoomMoreX) {
                        this.mMatrix.set(this.mSavedMatrix);
                        this.mMatrix.postScale(scaleX2, 1.0f, t.f72x, t.f73y);
                        if (l != null) {
                    ...
                    if (canZoomMoreY) {
                        this.mMatrix.set(this.mSavedMatrix);
                        this.mMatrix.postScale(1.0f, scaleY2, t.f72x, t.f73y);
                        if (l != null) {
    ...
    private static void midPoint(MPPointF point, MotionEvent event) {
        ...
        point.f72x = (event.getX(0) + event.getX(1)) / 2.0f;
        point.f73y = y / 2.0f;
    }
    ...
    public boolean onDoubleTap(MotionEvent e) {
        ...
        if (((BarLineChartBase) this.mChart).isDoubleTapToZoomEnabled() && ((BarLineScatterCandleBubbleData) ((BarLineChartBase) this.mChart).getData()).getEntryCount() > 0) {
            ...
            barLineChartBase.zoom(f, f2, trans.f72x, trans.f73y);
            if (((BarLineChartBase) this.mChart).isLogEnabled()) {
                Log.i("BarlineChartTouch", "Double-Tap, Zooming In, x: " + trans.f72x + ", y: " + trans.f73y);
            }
    ...
    public void stopDeceleration() {
        this.mDecelerationVelocity.f72x = 0.0f;
        this.mDecelerationVelocity.f73y = 0.0f;
    }

    public void computeScroll() {
        if (this.mDecelerationVelocity.f72x != 0.0f || this.mDecelerationVelocity.f73y != 0.0f) {
            long currentTime = AnimationUtils.currentAnimationTimeMillis();
            MPPointF mPPointF = this.mDecelerationVelocity;
            mPPointF.f72x = ((BarLineChartBase) this.mChart).getDragDecelerationFrictionCoef() * mPPointF.f72x;
            MPPointF mPPointF2 = this.mDecelerationVelocity;
            mPPointF2.f73y = ((BarLineChartBase) this.mChart).getDragDecelerationFrictionCoef() * mPPointF2.f73y;
            float timeInterval = ((float) (currentTime - this.mDecelerationLastTime)) / 1000.0f;
            float distanceX = this.mDecelerationVelocity.f72x * timeInterval;
            float distanceY = this.mDecelerationVelocity.f73y * timeInterval;
            this.mDecelerationCurrentPoint.f72x += distanceX;
            this.mDecelerationCurrentPoint.f73y += distanceY;
            MotionEvent event = MotionEvent.obtain(currentTime, currentTime, 2, this.mDecelerationCurrentPoint.f72x, this.mDecelerationCurrentPoint.f73y, 0);
            performDrag(event);
            event.recycle();
            ...
            if (((double) Math.abs(this.mDecelerationVelocity.f72x)) >= 0.01d || ((double) Math.abs(this.mDecelerationVelocity.f73y)) >= 0.01d) {
                Utils.postInvalidateOnAnimation(this.mChart);
                return;
            }
            ((BarLineChartBase) this.mChart).calculateOffsets();
