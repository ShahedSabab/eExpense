...
public class BarHighlighter extends ChartHighlighter<BarDataProvider> {
    ...
    public Highlight getHighlight(float x, float y) {
        ...
        if (set.isStacked()) {
            return getStackedHighlight(high, set, (float) pos.f70x, (float) pos.f71y);
        }
    ...
    public Highlight getStackedHighlight(Highlight high, IBarDataSet set, float xVal, float yVal) {
        BarEntry entry = (BarEntry) set.getEntryForXPos(xVal);
        if (entry == null) {
            return null;
        }
        ...
        MPPointD pixels = ((BarDataProvider) this.mChart).getTransformer(set.getAxisDependency()).getPixelsForValues(high.getX(), ranges[stackIndex].f61to);
        Highlight stackedHigh = new Highlight(entry.getX(), entry.getY(), (float) pixels.f70x, (float) pixels.f71y, high.getDataSetIndex(), stackIndex, high.getAxis());
