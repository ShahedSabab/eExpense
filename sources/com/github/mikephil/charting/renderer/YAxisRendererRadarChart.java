    ...
    public void renderAxisLabels(Canvas c) {
        if (this.mYAxis.isEnabled() && this.mYAxis.isDrawLabelsEnabled()) {
            ...
            while (j < labelCount && (j != labelCount - 1 || this.mYAxis.isDrawTopYLabelEntryEnabled())) {
                ...
                c.drawText(this.mYAxis.getFormattedLabel(j), pOut.f72x + 10.0f, pOut.f73y, this.mAxisLabelPaint);
                j++;
            }
            ...
            for (int i = 0; i < limitLines.size(); i++) {
                ...
                if (l.isEnabled()) {
                    ...
                    for (int j = 0; j < ((IRadarDataSet) ((RadarData) this.mChart.getData()).getMaxEntryCountSet()).getEntryCount(); j++) {
                        ...
                        if (j == 0) {
                            limitPath.moveTo(pOut.f72x, pOut.f73y);
                        } else {
                            limitPath.lineTo(pOut.f72x, pOut.f73y);
                        }
                    }
                    limitPath.close();
